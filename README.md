...
菜单会提供“开始做题”和“每日推荐”等入口，输入 `7` 可进入删除做题记录页面，输入 `0` 退出；命令行子命令仍保留用于脚本调用。
...
记录使用 `attempt_type`（`initial`/`review`/`practice`）区分首次尝试和复习，
并保存 FSRS 的 `retrievability`；旧数据库会在启动时自动迁移，旧的 `is_initial`
会被迁移后删除，不再用于业务判断。自动评分依次考虑题解、错误提交和记忆新鲜度，
再用难度下限 L0、用户 EWMA L1、题目 EWMA L2 三层基线计算用时 excess。
L1 使用跨题池化样本，L2 同时包含 initial 与 review；半衰期、池化最小题数、
新鲜度上限及 excess 参数位于 `config.yaml`，显式传入 `--score` 始终覆盖自动评分。
